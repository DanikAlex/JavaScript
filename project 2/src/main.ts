...
import addCoinToArray, { closeToggleLimit } from './toggleCoins.js';
...
async function displayCoinData(e: MouseEvent) {
...
async function coinSearch(): Promise<void> {
    ...
    try {
        ...
        const filteredCoins = coins.filter((coin) => 
            coin.symbol.toLowerCase().includes(coinSymbol)
        );
...
function limitSwitches() {
    ...
    switches.forEach(switchEl => {
        switchEl.addEventListener('change', () => {
            const checkedSwitches = Array.from(switches).filter(sw => (sw as HTMLInputElement).checked);
            const canToggle = checkedSwitches.length < maxActiveSwitches;

            switches.forEach(sw => {
                const inputElement = sw as HTMLInputElement;
